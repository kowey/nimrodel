...
%                                                                           %
% File:            arity.pl                                                 %
% Purpose:         DATR built-in predicates - Arity prolog version          %
% Author:          Roger Evans, Petra Naerger & Ingrid Renz                 %
%                                                                           %
...
%   Arity prolog. Copy this file to builtin.pl in the DATR directory.

db_asserta(X) :- asserta(X).        % assert before other clauses
db_assertz(X) :- assertz(X).        % assert after other clauses

db_retract(X)         :- retract(X),!.   % retract X
db_retract((X:-true)) :- !,              % retract facts which match
                         retract(X).     % clause(X,true)

db_retractall(G) :- db_retract(G),fail.     % remove all clauses matching G
db_retractall(G).
...
db_get0(26).                        % arity's get0 fails at eof
...
% $Id: arity.pl 1.2 1994/07/14 22:06:28 rpe Release $
